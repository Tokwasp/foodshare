...
## 1. 고민의 시작
**"메일을 전송하는 도중 타임아웃이 발생하면 어떻게 될까?"** <br><br>
메일 발송은 트랜잭션이 적용되어 있는 상황에서 외부 SMTP 서버에 메일 전송을 했을 때 응답이 없을경우 타임 아웃이 발생합니다. <br>
타임 아웃이 발생하기 전까지 DB 커넥션이 낭비 되지않을까? 의문이 생겼습니다.
...
[트랜잭션 종료] → 그제서야 커넥션 반납
```
- 만약 메일 전송이 지연되다면 DB 커넥션은 반납되지 못하여 시스템 성능 저하를 발생 시킬 수 있다고 생각했습니다.
