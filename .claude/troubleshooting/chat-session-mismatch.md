...
## 3. 구독 유형
- **userId 단위 구독**: 방에 메시지 발행 시 인원 수만큼 발행으로 인한 비효율
- **room 단위 구독**: WebSocket이 연결/재연결될 때마다 그 회원이 속한 모든 방을 구독 ( DB 조회 )으로 인한 비효율
- **단일 토픽 구독**: 모든 인스턴스가 메시지를 1회 수신 + 비 로그인 알림 불가
userId, romm 구독은 비효율적인 위 처럼 비효율적인 측면이 있다고 생각하여 단일 토픽 구독을 선택했습니다.
