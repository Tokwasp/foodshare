...
### 3-2. 비로그인 알림 
Reids pub/sub 사용 시 비로그인 유저에게 알림을 보내는 서버를 확정할 수 없습니다.
대표적인 pub/sub 구독 유형 3가지 예시를 통해 알아보겠습니다.
- **userId 단위 구독**: 방에 메시지 발행 시 인원 수만큼 발행으로 인한 비효율 + 비 로그인 알림 불가능
- **room 단위 구독**: WebSocket이 연결/재연결될 때마다 그 회원이 속한 모든 방을 구독 ( DB 조회 )으로 인한 비효율 + 비 로그인 알림 불가
- **단일 토픽 구독**: 모든 인스턴스가 메시지를 1번 발행 받으면 되지만 역시 비 로그인 알림 불가
비로그인 회원에게 알림을 확정적으로 보내기 위한 경우에는 메시징 큐가 필요합니다.
